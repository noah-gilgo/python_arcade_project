...
class Spell:
    ...
    def cast_spell(self, caster, targeted_characters, controller):
        ...
        if self.time_before_animation_begins is not None:
            pyglet.clock.schedule_once(lambda dt: self.animate_spell(caster, targeted_characters, controller.sprites_and_effects_collection), self.time_before_animation_begins)
        if self.time_before_target_affected_by_spell is not None:
            pyglet.clock.schedule_once(lambda dt: self.affect_targets_with_spell(caster, targeted_characters, controller), self.time_before_target_affected_by_spell)
