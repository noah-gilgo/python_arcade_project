...
from arcade import Sprite, Texture, LRBT, Rect, SpriteSheet, LBWH
...
class SparkleAnimation(MultiSpriteAnimation):
    def __init__(self, target: Sprite, total_duration: float = 1.0, color: Color = None,
        ...
        if target:
            ...
            if particle_starting_rect:
                ...
                self.spare_particle_min_x = int(self.target.center_x - (self.target.width / 4))
                self.spare_particle_max_x = int(self.target.center_x + (self.target.width / 4))

                self.spare_particle_min_y = int(self.target.center_y - (self.target.height / 2))
                self.spare_particle_max_y = int(self.target.center_y + (self.target.height / 2))
            ...
            super().__init__(
                sprites=self.particle_sprite_list,
