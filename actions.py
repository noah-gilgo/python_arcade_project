...
class SpellAction(Action):
    ...
    def execute(self):
        ...
        if self.spell.is_friendly_spell:
            ...
            player_or_enemies_list = self.controller.enemies

        targeted_characters = []
        if len(self.targets) == 1:
            if self.targets[0] not in player_or_enemies_list:
                if len(player_or_enemies_list) > 0:
                    self.targets[0] = player_or_enemies_list[0]
        else:
            for target in self.targets:
                if target in player_or_enemies_list:
                    targeted_characters.append(target)
